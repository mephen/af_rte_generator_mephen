...
def gen_Rte_Cs_Data_Management_h():
    ...
    write_content.extend([
        ...
        f'Rte_Cs_metaData rte_cs_metaData_arr[{num_of_in_task_CS_comm}];',
        '',
...
def gen_Rte_Event_Cfg_h():
    ...
    write_content.extend([
        ...
        '    }',
        '',
        '#define TimingEvent 0x00',
        '#define BackgroundEvent 0x10',
        '#define DataReceivedEvent 0x20',
        '#define DataReceiveErrorEvent 0x30',
        '#define DataSendCompletedEvent 0x40',
        '#define DataWriteCompletedEvent 0x50',
        '#define OperationInvokedEvent 0x60',
        '#define AsynchronousServerCallReturnsEvent 0x70',
        '#define SwcModeSwitchEvent 0x80',
        '#define ModeSwitchedAckEvent 0x90',
        '#define SwcModeManagerErrorEvent 0xA0',
        '#define ExternalTriggerOccurredEvent 0xB0',
        '#define InternalTriggerOccurredEvent 0xC0',
        '#define InitEvent 0xD0',
        '#define TransformerHardErrorEvent 0xE0',
        '#define OsTaskExecutionEvent 0xF0',
        ...
        '#define get_trigger_runnable_type(event) (get_rteevent_type(event) == InitEvent)? (event->status_uint16 & 0x1800) : -1',
        ...
        '#define get_communication_type(event) (get_rteevent_type(event) == OperationInvokedEvent)? (event->status_uint16 & 0x1800) : -1',
    ...
    for task_name in used_task:
        ...
        for component in client_server_component_list:
            if(component['c/s-interface-invoke-type'] == in_task):
                ...
                if(component['CRR-task'].get_shortName() == task_name):  task_ev_len += 1
        ...
        for component in client_server_component_list:
            if(component['c/s-interface-invoke-type'] == in_task):
                if((component['start_event'] != None) and (component['CR-task'].get_shortName() == task_name)):
                    write_content.extend([
                        f'#define {component["start_event"].get_shortName()}_{task_name.lower()} {ev_index} ',
                    ])
                    ev_index += 1
                if((component['operationInvokedEvent'] != None) and (component['SR-task'].get_shortName() == task_name)):
                    write_content.extend([
                        f'#define {component["operationInvokedEvent"].get_shortName()}_{task_name.lower()} {ev_index} ',
                    ])
                    ev_index += 1
                if((component['AsynchronousServerCallReturnsEvent'] != None) and (component['CRR-task'].get_shortName() == task_name)):
                    write_content.extend([
                        f'#define {component["AsynchronousServerCallReturnsEvent"].get_shortName()}_{task_name.lower()} {ev_index} ',
                    ])
                    ev_index += 1
