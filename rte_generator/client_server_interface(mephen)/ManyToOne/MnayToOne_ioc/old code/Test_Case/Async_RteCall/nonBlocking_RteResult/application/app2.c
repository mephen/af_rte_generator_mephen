...
TASK(T11)//ioc server side: 接收request，處理request，發送response
{
    ...
    while(rte_event_t11[OperationInvokedEvent_3_t11]->rteevent || rte_event_t11[OperationInvokedEvent_4_t11]->rteevent){//use while instead of if. Because inter-partition communication is multi-core, so there may be multiple requests coming when the server is processing the request.
        if(rte_event_t11[OperationInvokedEvent_3_t11]->rteevent){
            GetLock(&lock, lock_bit);
            rte_event_t11[OperationInvokedEvent_3_t11]->rteevent--;
            ReleaseLock(&lock, lock_bit);
        }else if(rte_event_t11[OperationInvokedEvent_4_t11]->rteevent){
            GetLock(&lock, lock_bit);
        ...
        switch (client_id){
            case 3: //client runnable 3 發起的 request
                ...
                break;
            case 4: //client runnable 4 發起的 request
                GetLock(&lock, lock_bit);
