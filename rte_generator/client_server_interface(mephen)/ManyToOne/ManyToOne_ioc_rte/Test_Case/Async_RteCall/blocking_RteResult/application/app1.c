...
TASK(T01)//rte client side: 發送request。(發起 request 的 runnable 應該放在同一個 task，不然不知道該在哪裡 activate 這些含有發起 requests 的 runnables 的 task)
{
    ...
    if(rte_event_t01[InitEvent_1_t01]->rteevent){
        ...
        RTE_RUNNABLE_Client1();//send request info (parameters & transaction handle).
        ...
        uint16 result = RTE_RUNNABLE_ClientResponse1();
        /*test*/
        if(result == 10U){
            PrintText("success\r\n\0");
        }
    }
    if(rte_event_t01[InitEvent_2_t01]->rteevent){
        GetLock(&lock, lock_bit);
        rte_event_t01[InitEvent_2_t01]->rteevent--;
        ReleaseLock(&lock, lock_bit);

        RTE_RUNNABLE_Client2();
        //[SWS_Rte_02658]: 不可能調用 RTE_RUNNABLE_Client1()，因為現在 RTE_RUNNABLE_Client1 已經有一個 unfinished c/s request.

        uint16 result = RTE_RUNNABLE_ClientResponse2();
        /*test*/
        if(result == 15U){
            PrintText("success\r\n\0");
        }
    ...
    if(rte_event_t01[InitEvent_3_t01]->rteevent){
        GetLock(&lock, lock_bit);
        rte_event_t01[InitEvent_3_t01]->rteevent--;
        ReleaseLock(&lock, lock_bit);

        RTE_RUNNABLE_Client3();

        uint16 result = RTE_RUNNABLE_ClientResponse3();
        /*test*/
        if(result == 15U){
            PrintText("success\r\n\0");
        }
...
TASK(T02)
{
    ...
    while(rte_event_t02[OperationInvokedEvent_1_t02]->rteevent || rte_event_t02[OperationInvokedEvent_2_t02]->rteevent){
        if(rte_event_t02[OperationInvokedEvent_1_t02]->rteevent){
            GetLock(&lock, lock_bit);
            rte_event_t02[OperationInvokedEvent_1_t02]->rteevent--;
            ReleaseLock(&lock, lock_bit);
        }else if(rte_event_t02[OperationInvokedEvent_2_t02]->rteevent){
            GetLock(&lock, lock_bit);
    ...
    if(rte_event_t02[AsynchronousServerCallReturnsEvent_1_t02]->rteevent){
        ...
        RTE_Enqueue(&RB_response_CRR1, &server_response, sizeof(ResponseInfoType)); //RB_response_CRR1: response Q of CRR1

        SetEvent(T01, event1); //SetEvent() can't put in the server runnable, because client_side_task stills need to do some data processing before enqueuing server_response.
    }
    if(rte_event_t02[AsynchronousServerCallReturnsEvent_2_t02]->rteevent){
        ...
        RTE_Enqueue(&RB_response_CRR2, &server_response, sizeof(ResponseInfoType));
    ...
    if(rte_event_t02[AsynchronousServerCallReturnsEvent_3_t02]->rteevent){
        ...
        RTE_Enqueue(&RB_response_CRR3, &server_response, sizeof(ResponseInfoType));
