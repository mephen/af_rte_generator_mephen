...
RteResultMetaData Rte_Result_SWC_Client_Rport1_Blocking_Client_Response_2_metaData = {
    .transaction_handle = {
...
//2. metaData
Std_ReturnType Rte_Result_SWC_Client_Rport1_Blocking_Client_Response_2(Impl_uint16* response){
    ...
    if(Rte_Result_SWC_Client_Rport1_Blocking_Client_Response_2_metaData.connected_unconnected == "unconnected"){
        rte_error = RTE_E_UNCONNECTED;
    }
    if(rte_error == RTE_E_OK){
        if(Rte_Result_SWC_Client_Rport1_Blocking_Client_Response_2_metaData.in_exclusiveArea_or_not == "InExclusiveArea"){
            rte_error = RTE_E_IN_EXCLUSIVE_AREA;
        }
    }
    if(rte_error == RTE_E_OK){
        if(Rte_Result_SWC_Client_Rport1_Blocking_Client_Response_2_metaData.CanAccess == "CanNot"){
    ...
    if(rte_error == RTE_E_OK){
        ...
        if(rte_error == RTE_E_TIMEOUT){//timeout
            Rte_Result_SWC_Client_Rport1_Blocking_Client_Response_2_metaData.transaction_handle.sequence_counter++; //means that x-th Rte_call will be finished (x = sequence_counter)
        }else if(Rte_Result_SWC_Client_Rport1_Blocking_Client_Response_2_metaData.transaction_handle.sequence_counter > server_response.transaction_handle.sequence_counter){//outdated server_response, and discard it.
            rte_error = RTE_E_TIMEOUT; 
        }else if(Rte_Result_SWC_Client_Rport1_Blocking_Client_Response_2_metaData.transaction_handle.sequence_counter == server_response.transaction_handle.sequence_counter){//not outdated, and not timeout
            Rte_Result_SWC_Client_Rport1_Blocking_Client_Response_2_metaData.transaction_handle.sequence_counter++; 
