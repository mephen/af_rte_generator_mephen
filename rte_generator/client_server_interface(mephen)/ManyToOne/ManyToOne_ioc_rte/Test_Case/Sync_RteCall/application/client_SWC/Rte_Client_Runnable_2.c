...
Std_ReturnType Rte_Call_Port1_AsyncRteAdd_CR2(Impl_uint16 para_1, Impl_uint16 para_2){
    if(request_number_CR2 == 0){//one client can only has one request at a time.
        ...
        if(Rte_Call_Port1_AsyncRteAdd_CR2_metaData.connected_unconnected == "unconnected"){
            rte_error = RTE_E_UNCONNECTED;
        }
        if(rte_error == RTE_E_OK){
            if(Rte_Call_Port1_AsyncRteAdd_CR2_metaData.in_exclusiveArea_or_not == "InExclusiveArea"){
        ...
        if(rte_error == RTE_E_OK){
            if(Rte_Call_Port1_AsyncRteAdd_CR2_metaData.CanAccess == "CanNot"){
        ...
        if(rte_error == RTE_E_OK){
            ...
            uint8 transformer_error = Xfrm_transformer(&Rte_Call_Port1_AsyncRteAdd_CR2_metaData.transaction_handle, &RB_transformer_core0, &RB_transformer_core0.currentSize, para_1);
            rte_error = Check_Transformer_Error(transformer_error, rte_error);
        }
        //transform parameter1
        if(rte_error == RTE_E_OK){
            rte_error = Check_Transformer_Buffer(&RB_transformer_core0);
            uint8 transformer_error = Xfrm_transformer(&Rte_Call_Port1_AsyncRteAdd_CR2_metaData.transaction_handle, &RB_transformer_core0, &RB_transformer_core0.currentSize, para_2);
            rte_error = Check_Transformer_Error(transformer_error, rte_error);
        }
        ...
        if(rte_error == RTE_E_OK || rte_error == RTE_E_SOFT_TRANSFORMER_ERROR){
            ...
            RTE_Enqueue(&RB_requestInfo_core0, &Rte_Call_Port1_AsyncRteAdd_CR2_metaData.transaction_handle.sequence_counter, sizeof(uint16));

            Rte_Call_Port1_AsyncRteAdd_CR2_metaData.transaction_handle.sequence_counter++; //means that x-th Rte_call is finished (x = sequence_counter)
        }
...
Std_ReturnType Rte_Call_Port1_SyncRteAdd_CR2(Impl_uint16 para_1, Impl_uint16 para_2, Impl_uint16* response){
    if(request_number_CR2 == 0){//one client can only has one request at a time.
        request_number_CR2 = 1;
        ...
        if(Rte_Call_Port1_SyncRteAdd_CR2_metaData.connected_unconnected == "unconnected"){
            rte_error = RTE_E_UNCONNECTED;
        }
        if(rte_error == RTE_E_OK){
            if(Rte_Call_Port1_SyncRteAdd_CR2_metaData.in_exclusiveArea_or_not == "InExclusiveArea"){
                rte_error = RTE_E_IN_EXCLUSIVE_AREA;
            }
        }
        if(rte_error == RTE_E_OK){
            if(Rte_Call_Port1_SyncRteAdd_CR2_metaData.CanAccess == "CanNot"){
                rte_error = RTE_E_SEG_FAULT;
            }
        }
        //transform parameter1
        if(rte_error == RTE_E_OK){
            rte_error = Check_Transformer_Buffer(&RB_transformer_core0);
            uint8 transformer_error = Xfrm_transformer(&Rte_Call_Port1_SyncRteAdd_CR2_metaData.transaction_handle, &RB_transformer_core0, &RB_transformer_core0.currentSize, para_1);
            rte_error = Check_Transformer_Error(transformer_error, rte_error);
        }
        //transform parameter1
        if(rte_error == RTE_E_OK){
            rte_error = Check_Transformer_Buffer(&RB_transformer_core0);
            uint8 transformer_error = Xfrm_transformer(&Rte_Call_Port1_SyncRteAdd_CR2_metaData.transaction_handle, &RB_transformer_core0, &RB_transformer_core0.currentSize, para_2);
            rte_error = Check_Transformer_Error(transformer_error, rte_error);
        }
        ...
        if(rte_error == RTE_E_OK || rte_error == RTE_E_SOFT_TRANSFORMER_ERROR){
            ...
            Impl_uint16 transformed_para_1;
            RTE_Dequeue(&RB_transformer_core0, &transformed_para_1, sizeof(Impl_uint16));
            Impl_uint16 transformed_para_2;
            ...
            RTE_Enqueue(&RB_requestInfo_core0, &Rte_Call_Port1_SyncRteAdd_CR2_metaData.transaction_handle.sequence_counter, sizeof(uint16));
            ...
            if(rte_error == RTE_E_TIMEOUT){//timeout
                Rte_Call_Port1_SyncRteAdd_CR2_metaData.transaction_handle.sequence_counter++; //means that x-th Rte_call will be finished (x = sequence_counter)
            }else if(Rte_Call_Port1_SyncRteAdd_CR2_metaData.transaction_handle.sequence_counter > server_response.transaction_handle.sequence_counter){//outdated server_response, and discard it.
                rte_error = RTE_E_TIMEOUT; 
            }else if(Rte_Call_Port1_SyncRteAdd_CR2_metaData.transaction_handle.sequence_counter == server_response.transaction_handle.sequence_counter){//correct response
                Rte_Call_Port1_SyncRteAdd_CR2_metaData.transaction_handle.sequence_counter++; 
