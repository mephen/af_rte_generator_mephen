...
Std_ReturnType Rte_Result_Port1_NonBlocking_CRR3(Impl_uint16* response){
    ...
    if(Rte_Result_Port1_NonBlocking_CRR3_metaData.connected_unconnected == "unconnected"){
        rte_error = RTE_E_UNCONNECTED;
    }
    if(rte_error == RTE_E_OK){
        if(Rte_Result_Port1_NonBlocking_CRR3_metaData.in_exclusiveArea_or_not == "InExclusiveArea"){
            rte_error = RTE_E_IN_EXCLUSIVE_AREA;
        }
    }
    if(rte_error == RTE_E_OK){
        if(Rte_Result_Port1_NonBlocking_CRR3_metaData.CanAccess == "CanNot"){
    ...
    if(rte_error == RTE_E_OK){
        ...
        if(rte_error == RTE_E_TIMEOUT){//timeout
            Rte_Result_Port1_NonBlocking_CRR3_metaData.transaction_handle.sequence_counter++; //means that x-th Rte_call will be finished (x = sequence_counter)
        }else if(Rte_Result_Port1_NonBlocking_CRR3_metaData.transaction_handle.sequence_counter > server_response.transaction_handle.sequence_counter){//outdated server_response, and discard it.
            rte_error = RTE_E_TIMEOUT; 
        }else if(Rte_Result_Port1_NonBlocking_CRR3_metaData.transaction_handle.sequence_counter == server_response.transaction_handle.sequence_counter){//not outdated, and not timeout
            Rte_Result_Port1_NonBlocking_CRR3_metaData.transaction_handle.sequence_counter++; 
...
Std_ReturnType Rte_Result_Port1_Blocking_CRR3(Impl_uint16* response){
    ...
    if(rte_error == RTE_E_OK){
        if(Rte_Result_Port1_Blocking_CRR3_metaData.in_exclusiveArea_or_not == "InExclusiveArea"){
            rte_error = RTE_E_IN_EXCLUSIVE_AREA;
        }
    }
    if(rte_error == RTE_E_OK){
        if(Rte_Result_Port1_Blocking_CRR3_metaData.CanAccess == "CanNot"){
            rte_error = RTE_E_SEG_FAULT;
        }
    }
    if(rte_error == RTE_E_OK){
        ...
        //check if there is outdated response, or timeout occur.
        if(rte_error == RTE_E_TIMEOUT){//timeout
            Rte_Result_Port1_Blocking_CRR3_metaData.transaction_handle.sequence_counter++; //means that x-th Rte_call will be finished (x = sequence_counter)
        }else if(Rte_Result_Port1_Blocking_CRR3_metaData.transaction_handle.sequence_counter > server_response.transaction_handle.sequence_counter){//outdated server_response, and discard it.
            rte_error = RTE_E_TIMEOUT; 
        }else if(Rte_Result_Port1_Blocking_CRR3_metaData.transaction_handle.sequence_counter == server_response.transaction_handle.sequence_counter){//not outdated, and not timeout
            Rte_Result_Port1_Blocking_CRR3_metaData.transaction_handle.sequence_counter++; 
...
Impl_uint16 RTE_RUNNABLE_ClientResponse3(){
    ...
    Std_ReturnType rte_error = Rte_Result_Port1_NonBlocking_CRR3(&response);
    return response;
}
