...
static RteCallMetaData AsyncIocMultiply_metaData = {
    .transaction_handle = {
...
static RteCallMetaData SyncIocMultiply_metaData = {
    .transaction_handle = {
        .client_id = 3U,        //runnable ID, according to the configuration arxml
...
Std_ReturnType Rte_Call_Port1_AsyncIocMultiply(Impl_uint16 para_1, Impl_uint16 para_2){
    if(request_number_CR3 == 0){//one client can only has one request at a time.
        ...
        if(AsyncIocMultiply_metaData.connected_unconnected == "unconnected"){
            rte_error = RTE_E_UNCONNECTED;
        }
        if(rte_error == RTE_E_OK){
            if(AsyncIocMultiply_metaData.in_exclusiveArea_or_not == "InExclusiveArea"){
        ...
        if(rte_error == RTE_E_OK){
            if(AsyncIocMultiply_metaData.CanAccess == "CanNot"){
        ...
        if(rte_error == RTE_E_OK){
            ...
            uint8 transformer_error = Xfrm_transformer(&AsyncIocMultiply_metaData.transaction_handle, &RB_transformer_core0, &RB_transformer_core0.currentSize, para_1);
            rte_error = Check_Transformer_Error(transformer_error, rte_error);
        }
        //transform parameter1
        if(rte_error == RTE_E_OK){
            rte_error = Check_Transformer_Buffer(&RB_transformer_core0);
            uint8 transformer_error = Xfrm_transformer(&AsyncIocMultiply_metaData.transaction_handle, &RB_transformer_core0, &RB_transformer_core0.currentSize, para_2);
            rte_error = Check_Transformer_Error(transformer_error, rte_error);
        }
        ...
        if(rte_error == RTE_E_OK || rte_error == RTE_E_SOFT_TRANSFORMER_ERROR){
            ...
            IocSend_Q1_SND1(AsyncIocMultiply_metaData.transaction_handle.client_id);
            IocSend_Q1_SND1(AsyncIocMultiply_metaData.transaction_handle.sequence_counter);
            AsyncIocMultiply_metaData.transaction_handle.sequence_counter++; //means that x-th Rte_call is finished (x = sequence_counter)
        }
...
Std_ReturnType Rte_Call_Port1_SyncIocMultiply(Impl_uint16 para_1, Impl_uint16 para_2, Impl_uint16* response){
    if(request_number_CR3 == 0){//one client can only has one request at a time.
        request_number_CR3 = 1;
        ...
        if(SyncIocMultiply_metaData.connected_unconnected == "unconnected"){
            rte_error = RTE_E_UNCONNECTED;
        }
        if(rte_error == RTE_E_OK){
            if(SyncIocMultiply_metaData.in_exclusiveArea_or_not == "InExclusiveArea"){
                rte_error = RTE_E_IN_EXCLUSIVE_AREA;
            }
        }
        if(rte_error == RTE_E_OK){
            if(SyncIocMultiply_metaData.CanAccess == "CanNot"){
                rte_error = RTE_E_SEG_FAULT;
            }
        }
        //transform parameter1
        ...
        if(rte_error == RTE_E_OK || rte_error == RTE_E_SOFT_TRANSFORMER_ERROR){
            ...
            IocSend_Q1_SND1(transformed_para_1);
            IocSend_Q1_SND1(transformed_para_2);
            IocSend_Q1_SND1(SyncIocMultiply_metaData.transaction_handle.client_id);
            IocSend_Q1_SND1(SyncIocMultiply_metaData.transaction_handle.sequence_counter);
            ...
            if(rte_error == RTE_E_TIMEOUT){//timeout
                SyncIocMultiply_metaData.transaction_handle.sequence_counter++; //means that x-th Rte_call will be finished (x = sequence_counter)
            }else if(SyncIocMultiply_metaData.transaction_handle.sequence_counter > server_response.transaction_handle.sequence_counter){//outdated server_response, and discard it.
                rte_error = RTE_E_TIMEOUT; 
            }else if(SyncIocMultiply_metaData.transaction_handle.sequence_counter == server_response.transaction_handle.sequence_counter){//correct response
                SyncIocMultiply_metaData.transaction_handle.sequence_counter++;
...
Impl_uint16 RTE_RUNNABLE_Client3(){
    ...
    Std_ReturnType rte_error = Rte_Call_Port1_SyncIocMultiply(data_1, data_2, &response);
    return response;
}