...
uint8 lock_aa = 0;
uint8 lock_bit_for_T02_T11 = 1; 

uint8 T11_finished = 0;
...
void iocCase_client_side(){
    uint16 server_response;
    StatusType ioc_error_code = IocReceive_aa((int*)&server_response);
    ...
    check_RTE_E_TRANSFORMER_LIMIT(&transformer_RB_ioc, &errorFlag_Rte_Result_Port1_InterPartitionAsynchronousAdd_Blocking);
    ...
    check_RTE_TRANSFORMER_ERROR(&transformer_RB_ioc, transformer_error_code, &errorFlag_Rte_Result_Port1_InterPartitionAsynchronousAdd_Blocking);
...
TASK(T01)
{
    ...
    errorFlag_Rte_Result_Port1_InterPartitionAsynchronousAdd_Blocking = RTE_E_TIMEOUT;
    TerminateTask();
}
...
TASK(T02)
{
    GetLock(&lock_aa, lock_bit_for_T02_T11);
    ...
    ReleaseLock(&lock_aa, lock_bit_for_T02_T11);

    ActivateTask(T03);
    Impl_uint16 get_by_RteResult = Rte_Client_ResponseRunnable_1(transaction_handle.Async_rteCall_returnValue);
    if(get_by_RteResult == 10U){
        PrintText("success\r\n\0");
    }
...
TASK(T03)
{
    ...
    iocCase_client_side();
    PrintText("T03 set event to wake up T02\r\n\0");
    SetEvent(T02, event1);
    TerminateTask();
}
...
void iocCase_server_side(Rte_Cs_TransactionHandleType transaction_handle){
    uint16 data_1;
    uint16 data_2;
...
TASK(T11)
{
    ...
    GetLock(&lock_aa, lock_bit_for_T02_T11);//make sure T11 works after T02 initiates all the C/S operations.
    ReleaseLock(&lock_aa, lock_bit_for_T02_T11);
    ...
    switch(transaction_handle.communication_type){
        case 1: //com
            break;
        case 2: //ioc
            iocCase_server_side(transaction_handle);
            break;
        case 3: //rte
            break;
        default:
    ...
    T11_finished = 1;//make sure T03 works after T11 sends the server response.
    TerminateTask();
}