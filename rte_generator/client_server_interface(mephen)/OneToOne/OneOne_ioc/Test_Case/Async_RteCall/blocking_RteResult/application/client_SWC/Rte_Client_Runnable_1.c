...
Std_ReturnType Rte_Call_Port1_InterPartitionSynchronousAdd(Impl_uint16 data_1, Impl_uint16 data_2, Impl_uint16* response){
    ...
    if(rte_error_code == RTE_E_OK || rte_error_code == RTE_E_SOFT_TRANSFORMER_ERROR){
        ...
        if(errorFlag_Rte_Call_Port1_InterPartitionSynchronousAdd != RTE_E_OK){
            rte_error_code = errorFlag_Rte_Call_Port1_InterPartitionSynchronousAdd; //in ioc case: RTE_E_TIMEOUT, RTE_E_TRANSFORMER_LIMIT, RTE_E_HARD_TRANSFORMER_ERROR, or RTE_E_SOFT_TRANSFORMER_ERROR
        }
    }
    if(rte_error_code == RTE_E_OK || rte_error_code == RTE_E_SOFT_TRANSFORMER_ERROR){
        RTE_Dequeue(&Rte_response_RB_ioc, (void*)response, sizeof(Impl_uint16));
        Rte_Call_Port1_InterPartitionSynchronousAdd_metaData.transaction_handle.sequence_counter++;//means rte_call is finished successfully
    }
    if(Enqueue_rte_call_RB == 1){
...
Rte_Cs_TransactionHandleType Rte_Client_Runnable_1(){
    //for developer to design the implementation
    Impl_uint16 data_1 = 4U;
    Impl_uint16 data_2 = 6U;
    Std_ReturnType rte_error_code = Rte_Call_Port1_InterPartitionAsynchronousAdd(data_1, data_2);
    Rte_Call_Port1_InterPartitionAsynchronousAdd_metaData.transaction_handle.Async_rteCall_returnValue = rte_error_code;
    return Rte_Call_Port1_InterPartitionAsynchronousAdd_metaData.transaction_handle;//check return value in test code. can be removed in real situation.(void Rte_Client_Runnable())
}