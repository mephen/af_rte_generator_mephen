...
extern Std_ReturnType errorFlag_Rte_Call_Port1_InterEcuSynchronousAdd;
...
void comCase_client_side(){
    uint8 com_error_code = Com_ReceiveSignal(0U, &(signal)errorFlag_Rte_Call_Port1_InterEcuSynchronousAdd);
...
TASK(T01)
{
    ...
    //in the priority rule of rte_call, the priority of RTE_E_TIMEOUT is lower than the hard transformer error & com error
    if(errorFlag_Rte_Call_Port1_InterEcuSynchronousAdd != RTE_E_HARD_TRANSFORMER_ERROR || errorFlag_Rte_Call_Port1_InterEcuSynchronousAdd != RTE_E_COM_BUSY || errorFlag_Rte_Call_Port1_InterEcuSynchronousAdd != RTE_E_COM_STOPPED){
        errorFlag_Rte_Call_Port1_InterEcuSynchronousAdd = RTE_E_TIMEOUT;
    }
    TerminateTask();
}
