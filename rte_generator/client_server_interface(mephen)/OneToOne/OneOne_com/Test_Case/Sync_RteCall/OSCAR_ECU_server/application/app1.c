...
Std_ReturnType errorFlag_Rte_Call_Port1_InterEcuSynchronousAdd;
...
void check_com_error(uint8 com_error_code){
    if(com_error_code == COM_SERVICE_NOT_AVAILABLE){
        errorFlag_Rte_Call_Port1_InterEcuSynchronousAdd = RTE_E_COM_STOPPED;
    }else if (com_error_code == COM_BUSY)
    {
        errorFlag_Rte_Call_Port1_InterEcuSynchronousAdd = RTE_E_COM_BUSY;
    }
    return;
}

void comCase_server_side(){
    signal signal_1;
    signal signal_2;
    uint8 com_error_code = Com_ReceiveSignal(1U, &signal_1);
    check_com_error(com_error_code);
    
    com_error_code = Com_ReceiveSignal(2U, &signal_2);
    check_com_error(com_error_code);
    ...
    check_RTE_E_TRANSFORMER_LIMIT(&transformer_RB_com, &errorFlag_Rte_Call_Port1_InterEcuSynchronousAdd);
    uint8 transformer_error_code = Xfrm_Inv_transformer_com(&transaction_handle, &transformer_RB_com, &transformer_RB_com.currentSize, signal_1);
    check_RTE_TRANSFORMER_ERROR(&transformer_RB_com, transformer_error_code, &errorFlag_Rte_Call_Port1_InterEcuSynchronousAdd);
    Impl_uint16 signal_1_transformed;
    RTE_Dequeue(&transformer_RB_com, (void*)&signal_1_transformed, sizeof(Impl_uint16));

    check_RTE_E_TRANSFORMER_LIMIT(&transformer_RB_com, &errorFlag_Rte_Call_Port1_InterEcuSynchronousAdd);
    uint8 transformer_error_code = Xfrm_Inv_transformer_com(&transaction_handle, &transformer_RB_com, &transformer_RB_com.currentSize, signal_2);
    check_RTE_TRANSFORMER_ERROR(&transformer_RB_com, transformer_error_code, &errorFlag_Rte_Call_Port1_InterEcuSynchronousAdd);
    Impl_uint16 signal_2_transformed;
    RTE_Dequeue(&transformer_RB_com, (void*)&signal_2_transformed, sizeof(Impl_uint16));

    /*call server runnable*/
    Impl_uint16 server_response = Rte_Server_Runnable(signal_1_transformed, signal_2_transformed);

    /*transform server response*/
    check_RTE_E_TRANSFORMER_LIMIT(&transformer_RB_com, &errorFlag_Rte_Call_Port1_InterEcuSynchronousAdd);
    uint8 transformer_error_code = Xfrm_transformer_2_com(&transaction_handle, &transformer_RB_com, &transformer_RB_com.currentSize, server_response);
    check_RTE_TRANSFORMER_ERROR(&transformer_RB_com, transformer_error_code, &errorFlag_Rte_Call_Port1_InterEcuSynchronousAdd);
    signal server_response_transformed;
    RTE_Dequeue(&transformer_RB_com, (void*)&server_response_transformed, sizeof(uint16));

    com_error_code = Com_SendSignal(3U, &server_response_transformed);
    check_com_error(com_error_code);

    signal error_flag = (signal)errorFlag_Rte_Call_Port1_InterEcuSynchronousAdd;
    com_error_code = Com_SendSignal(4U, &error_flag);
    check_com_error(com_error_code);
