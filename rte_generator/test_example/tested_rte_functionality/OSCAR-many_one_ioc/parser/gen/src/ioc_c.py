...
def generate_IocRead_receiverId(ioc, iocid, iocsenders, iocDataProperties):
    i = 1
    data = ""
    Is_pointer = False
    for iocData in iocDataProperties:
...
if ioc != False:
    ...
    for iocid in iocids:
        ...
        iocsenders = config.getList("/AUTOSAR/" + ioc + "/" + iocid ,"OsIocSenderProperties")
        iocreceivers = config.getList("/AUTOSAR/" + ioc + "/" + iocid ,"OsIocReceiverProperties")
        length = config.getValue("/AUTOSAR/" + ioc + "/" + iocid ,"OsIocBufferLength") #改動：多加這一行用來修正 "event" 和 "data" sementics 的判斷處

        if length != False: #改動：多加這一行用來修正 "event" 和 "data" sementics 的判斷處
            if len(iocDataProperties) == 1:
                # IocSend_senderId
        ...
        #改動：IocWrite 和 IocSend 的差異是 OsIocBufferlength 有無設定。判斷是應該是確認 len(OsIocBufferLength)
        else:
            if len(iocDataProperties) == 1:
                # IocWrite_senderId
