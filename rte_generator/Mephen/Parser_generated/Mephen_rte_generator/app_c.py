...
from app_h import intra_partition, inter_partition, inter_ECU, dataType_mappingSets, client_server_component_list\
    ...
    ,used_app, task_to_app_mapping, check_server, event_to_task_mapping, ev_dict, ev_order_list

def gen_RteCall_relatedDT_CR(rable, write_content, rable_name, scp_keys):
    CS_component = None
...
def gen_RteResult_relatedDT_CRR(rable, write_content, rable_name, async_scrp_keys):
    CS_component = None
    for component in client_server_component_list:
...
def gen_serverOP_relatedDT_SR(rable, event_of_swc, write_content, rable_name):
    server_ev = None
...
def gen_app_c(swc):
    ...
    for rable in rables_of_swc:
        ...
        #生成 RTE-APIs
        #   Life Cycle：
        #       生在 Rte_Main.h，並且是由 EcucM 調用，而非 runnable 調用
        #       Only concern the order of invoking runnables and don't need to know which RTE-APIs will be used in the runnables.
        #       Therefore, the generator only generates the runnable in runnable.c .
        #   C/S interface：
        ...
        if(len(scp_keys) > 0): #client runnable
            ...
            gen_RteCall_relatedDT_CR(rable, write_content, rable_name, scp_keys)
        ...
        if(len(async_scrp_keys) > 0): #client_response runnable
            ...
            gen_RteResult_relatedDT_CRR(rable, write_content, rable_name, async_scrp_keys)
        ...
        if(check_server(event_of_swc, rable)): #server runnable
            ...
            gen_serverOP_relatedDT_SR(rable, event_of_swc, write_content, rable_name)

        #生成 Runnables
        write_content.extend([
            f'void RTE_RUNNABLE_{rable_name}()' '{',
            '    //for developer to design the implementation',
            fr'    PrintText("{rable_name}\r\n\0");',
            '    return;',
            '}',
        ])
...
def gen_Rte_Event_Cfg_c():
    ...
    for task_name in task_to_app_mapping:
        ...
        arr_ele_Background_ev = ''
        arr_ele_Timing_ev = ''
        arr_ele_Init_ev = ''
        arr_ele_Modeswitch_ev = ''
        arr_ele_OpInvok_ev = ''
        arr_ele_AsyncReturn_ev = ''

        for ev_order_item in ev_order_list:
            ev_path = ev_order_item.event_path
            ...
            if(event_to_task_mapping[ev_name].task_name == task_name):
                rable = ev_dict[ev_path].get_startOnEvent()
                rable_name = rable.get_shortName()
                # print(rable_name)
                if(type(ev_dict[ev_path]) == BackgroundEvent):
                    ev_Background_def += f'RteEvent {ev_name} = ' + '{' + f'(uint16)BackgroundEvent, {rable_name}' + '};\n'
                    arr_ele_Background_ev += f'    &{ev_name}, \n'
                elif(type(ev_dict[ev_path]) == TimingEvent):
                    ev_Timing_def += f'RteEvent {ev_name} = ' + '{' + f'(uint16)TimingEvent, {rable_name}' + '};\n'
                    arr_ele_Timing_ev += f'    &{ev_name}, \n'
                elif(type(ev_dict[ev_path]) == InitEvent):
                    ev_Init_def += f'RteEvent {ev_name} = ' + '{' + f'(uint16)InitEvent, {rable_name}' + '};\n'
                    arr_ele_Init_ev += f'    &{ev_name}, \n'
                elif(type(ev_dict[ev_path]) == SwcModeSwitchEvent):
                    ev_Modeswitch_def += f'RteEvent {ev_name} = ' + '{' + f'(uint16)SwcModeSwitchEvent, {rable_name}' + '};\n'
                    arr_ele_Modeswitch_ev += f'    &{ev_name}, \n'
                elif(type(ev_dict[ev_path]) == OperationInvokedEvent):
                    for component in client_server_component_list:
                        if(component['operationInvokedEvent'] == ev_dict[ev_path] and component['c/s-interface-invoke-type'] == in_task):
                            ev_OpInvok_def += f'RteEvent {ev_name} = ' + '{' + f'(uint16)OperationInvokedEvent + {component["comm-type"]}, {rable_name}' + '};\n'
                            break
                    arr_ele_OpInvok_ev += f'    &{ev_name}, \n'
                elif(type(ev_dict[ev_path]) == AsynchronousServerCallReturnsEvent):
                    for component in client_server_component_list:
                        if(component['AsynchronousServerCallReturnsEvent'] == ev_dict[ev_path] and component['c/s-interface-invoke-type'] == in_task):
                            ev_AsyncReturn_def += f'RteEvent {ev_name} = ' + '{' + f'(uint16)AsynchronousServerCallReturnsEvent + {component["comm-type"]}, {rable_name}' + '};\n'
                            break
                    arr_ele_AsyncReturn_ev += f'    &{ev_name}, \n'
                arr_len += 1
        
        write_content.extend([
            ev_Background_def + ev_Timing_def + ev_Init_def + ev_Modeswitch_def + ev_OpInvok_def + ev_AsyncReturn_def + \
            f'RteEvent* rte_event_{task_name.lower()}[{arr_len}] = ' + '{',
            arr_ele_Background_ev + arr_ele_Timing_ev + arr_ele_Init_ev + arr_ele_Modeswitch_ev + arr_ele_OpInvok_ev + arr_ele_AsyncReturn_ev + \
            '};',
            '',
        ])

        # #RteEvent which trigger clients, operationInvokedEvent, AsynchronousServerCallReturnsEvent
        # if(client_server_component_list != []):
        #     for component in client_server_component_list:
        ...
        #                 #event definition
        #                 ev_start_initial += f'RteEvent {ev_name} = ' '{' f'(uint16){ev_type}+0x1, {component["CR"].get_shortName()}' '}; //assume the event is already triggered by Rte LifeCycle api \n'
        #                 #task event array ele       
